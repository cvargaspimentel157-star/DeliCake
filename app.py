...
from controladores.models import FooterConfig, db, Usuario, Cliente, Administrador, Categoria, Producto, Reseña, Suscriptor,Pedido
...
# Configurar mi SECRET KEY para que session funcione correctamente
...
def home():
    ...
    productos_destacados = (
        ...
        .order_by(func.count(Pedido.ID_Producto).desc())
        .limit(10)
        .all()
    )
    productos_destacados = [p[0] for p in productos_destacados]
...
def verificar_codigo():
    if request.method == "POST":
        ...
        if str(codigo_ingresado) == str(session.get("reset_codigo")):
...
def inject_footer():
    ...
    if footer:
        return {
            "horario_1": footer.horario_1,
            "horario_2": footer.horario_2,
        ...
        return {
            "horario_1": "Lunes - Sábados: 8am - 5pm",
            "horario_2": "Domingos y festivos: 9am - 2pm",
            "whatsapp_url": "https://wa.me/573178563246",
