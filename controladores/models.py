...
class Producto(db.Model):
    ...
    detalles_pedido = db.relationship("DetallePedido", back_populates="producto", cascade="all, delete-orphan")
...
class Pedido(db.Model):
    ...
    Estado_Pedido = db.Column(db.Enum('Pendiente', 'En proceso', 'Enviado', 'Entregado', 'Cancelado'), default='Pendiente')
    Total = db.Column(db.Numeric(10, 2))
    Metodo_Pago = db.Column(db.Enum('Tarjeta', 'Nequi', 'Daviplata', 'Efectivo'), nullable=True)
    ID_Cliente = db.Column(db.Integer, db.ForeignKey("Cliente.ID_cliente"), nullable=False)
    ID_Producto = db.Column(db.Integer, db.ForeignKey("Producto.ID_Producto"), nullable=False)

    cliente = db.relationship("Cliente", back_populates="pedidos")
    producto = db.relationship("Producto", back_populates="pedidos")
    detalles_pedido = db.relationship("DetallePedido", back_populates="pedido", cascade="all, delete-orphan")
    personalizaciones = db.relationship("PersonalizacionProducto", back_populates="pedido", cascade="all, delete-orphan")
    pagos = db.relationship("Pago", back_populates="pedido", cascade="all, delete")
...
class PersonalizacionProducto(db.Model):
    ...
    pedido = db.relationship("Pedido", back_populates="personalizaciones")
...
class Pago(db.Model):
    ...
    Estado_Pago = db.Column(db.Enum('Pendiente', 'Aprobado', 'Rechazado'), default='Pendiente')
    fecha_pago = db.Column(db.Date)
    Mensaje_Cliente = db.Column(db.String(255), default="Espera un momento, el administrador está revisando") 
...
class FooterConfig(db.Model):
    id = db.Column(db.Integer, primary_key=True)

    horario_1 = db.Column(db.String(200))
    horario_2 = db.Column(db.String(200))
